...
impl Upscaler {
    pub fn new(config: &Config) -> Self {
        Self {
            models_path: config.models_path.to_string(),
            model: config.model.to_string(),
            executable: config.executable.to_string(),
        }
    }

    pub fn upscale(&self, input: &str, output: &str, verbose: bool) -> Result<(), UpscaleError> {
        let mut proc = Command::new(&self.executable);
        if !verbose {
